...
def run():
    # 1. Apuntas directo al archivo específico
    ruta_archivo = "./networks/graph.txt"
    
    # Creas la carpeta de salida si no existe
    os.makedirs("./outs", exist_ok=True)

    # 2. Verificas si existe y procesas DIRECTO (sin bucles)
    if os.path.isfile(ruta_archivo):
        print(f"Procesando únicamente: {ruta_archivo}")
        
        num_nodos, aristas, uni, bi, conteo = procesar_pares(ruta_archivo)

        # Extraes el nombre ("graph") para el archivo de salida
        ...
        print(f"Error: No encontré el archivo en {ruta_archivo}")
